...
const MaterialPage = ({ setSidebarOpen }) => {
  ...
  useEffect(() => {
    loadMaterials().catch((e) => alert(e.message));
  }, []);
  ...
  const handleSubmit = (e) => {
    ...
    (async () => {
      if (editingMaterial?.id) {
        const res = await api.patch(`/api/materials/${editingMaterial.id}`, materialData);
          ...
          <div className="max-w-[1400px] mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-slate-500 text-sm mt-1">
                {loading ? 'Loading...' : `${filteredMaterials.length} of ${materials.length} materials`}
              </p>
            </div>
            ...
            <TableViewMaterial 
              ...
              onAdd={(newMaterial) => {
                const payload = {
                  ...
                  series: newMaterial.series,
                  pole: newMaterial.pole,
                  ka: newMaterial.ka,
                  ampere: newMaterial.ampere,
                ...
                api
                  .post('/api/materials', payload)
                  .then((res) => setMaterials((prev) => [res.data, ...prev]))
